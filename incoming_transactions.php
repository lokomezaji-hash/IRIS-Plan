...
$date_today = date('Ymd');
$prefix = "INC-" . $date_today . "-";

// Find the highest sequence tracking number saved today
$res = $conn->query("SELECT tracking_number FROM incoming WHERE tracking_number LIKE '$prefix%' ORDER BY tracking_number DESC LIMIT 1");

if ($res && $res->num_rows > 0) {
    $row = $res->fetch_assoc();
    // Extract the last 3 digits from the tracking number (e.g., "003" from "INC-20260526-003")
...
if (isset($_POST['add_document'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO incoming (tracking_number, subject, doc_type, doc_date, action_taken, remarks, file_path, uploaded_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
$where_clause = "";
if (!empty($search)) {
    $where_clause = " WHERE incoming.tracking_number LIKE '%$search%' 
                      OR incoming.subject LIKE '%$search%' 
                      OR incoming.doc_type LIKE '%$search%' 
                      OR incoming.action_taken LIKE '%$search%' 
                      OR user.name LIKE '%$search%'";
}

$count_sql = "SELECT COUNT(*) as total FROM incoming LEFT JOIN user ON incoming.uploaded_by = user.id" . $where_clause;
...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="incoming_transactions.php" class="active">Incoming Transactions</a>
        <a href="#">About Us</a>
        ...
        <div class="main-content">
            ...
            <div class="table-section">
                ...
                <form method="GET" action="" class="search-container">
                    ...
                    <?php if(!empty($search)): ?>
                        <a href="incoming_transactions.php" style="padding:8px; background:#6c757d; color:white; text-decoration:none; border-radius:4px; font-size:14px;">Clear</a>
                    <?php endif; ?>
                </form>
                ...
                <table>
                    ...
                    <tbody>
                        <?php 
                        $sql = "SELECT incoming.*, user.name as uploader_name, user.profile_pic as uploader_pic 
                                FROM incoming 
                                LEFT JOIN user ON incoming.uploaded_by = user.id 
                                $where_clause 
                                ORDER BY incoming.id DESC LIMIT $limit OFFSET $offset";
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php endif; ?>
                                <a href="delete_doc.php?id=<?php echo $row['id']; ?>" class="btn-sm btn-delete" onclick="return confirm('Are you sure?')">Delete</a>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" action="edit_doc_inc.php">
    ...
    <script>
        ...
        function openHistoryModal(id) {
            ...
            fetch('get_history_inc.php?id=' + id)
