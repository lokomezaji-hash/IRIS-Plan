...
$date_today = date('Ymd');
$prefix = "INT-" . $date_today . "-";

// Find the highest sequence tracking number saved today
$res = $conn->query("SELECT tracking_number FROM internal WHERE tracking_number LIKE '$prefix%' ORDER BY tracking_number DESC LIMIT 1");

if ($res && $res->num_rows > 0) {
    $row = $res->fetch_assoc();
    // Extract the last 3 digits from the tracking number (e.g., "003" from "INT-20260526-003")
...
if (isset($_POST['add_document'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO internal (tracking_number, subject, doc_type, doc_date, action_taken, remarks, file_path, uploaded_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
$where_clause = "";
if (!empty($search)) {
    $where_clause = " WHERE internal.tracking_number LIKE '%$search%' 
                      OR internal.subject LIKE '%$search%' 
                      OR internal.doc_type LIKE '%$search%' 
                      OR internal.action_taken LIKE '%$search%' 
                      OR user.name LIKE '%$search%'";
}

$count_sql = "SELECT COUNT(*) as total FROM internal LEFT JOIN user ON internal.uploaded_by = user.id" . $where_clause;
...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="internal_transactions.php" class="active">Internal Transactions</a>
        ...
        <div class="main-content">
            ...
            <div class="table-section">
                ...
                <form method="GET" action="" class="search-container">
                    ...
                    <?php if(!empty($search)): ?>
                        <a href="internal_transactions.php" style="padding:8px; background:#6c757d; color:white; text-decoration:none; border-radius:4px; font-size:14px;">Clear</a>
                    <?php endif; ?>
                </form>
                ...
                <table>
                    ...
                    <tbody>
                        <?php 
                        $sql = "SELECT internal.*, user.name as uploader_name, user.profile_pic as uploader_pic 
                                FROM internal 
                                LEFT JOIN user ON internal.uploaded_by = user.id 
                                $where_clause 
                                ORDER BY internal.id DESC LIMIT $limit OFFSET $offset";
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" action="edit_doc.php">
