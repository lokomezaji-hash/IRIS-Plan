...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT subject, action_taken, remarks FROM outgoing WHERE id = ?");
    ...
    $log_stmt = $conn->prepare("INSERT INTO outgoing_history (outgoing_id, old_subject, old_action, old_remarks, edited_by) VALUES (?, ?, ?, ?, ?)");
    ...
    $update_stmt = $conn->prepare("UPDATE outgoing SET subject = ?, action_taken = ?, remarks = ? WHERE id = ?");
