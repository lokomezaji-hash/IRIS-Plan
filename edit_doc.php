...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT subject, action_taken, remarks FROM internal WHERE id = ?");
    ...
    $log_stmt = $conn->prepare("INSERT INTO internal_history (internal_id, old_subject, old_action, old_remarks, edited_by) VALUES (?, ?, ?, ?, ?)");
    ...
    $update_stmt = $conn->prepare("UPDATE internal SET subject = ?, action_taken = ?, remarks = ? WHERE id = ?");
